...
const ChatWindow = ({ selectedChat, currentUserId, onSendMessage, isMobile = false }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0a0a]/50">
      ...
      <div
        ref={scrollRef}
        className={`flex-1 overflow-y-auto space-y-6 custom-scrollbar ${isMobile ? 'px-4 py-6' : 'px-6 py-8'}`}
      >
          ...
          selectedChat.messages.map((msg, index) => {
            ...
            return (
              <div key={index} className={`flex flex-col ${isCurrentUser ? "items-end" : "items-start"}`}>
                ...
                <div
                  className={`max-w-[80%] ${isMobile ? 'max-w-[85%]' : 'md:max-w-[70%]'} p-3 ${isMobile ? 'p-3' : 'p-4'} text-sm relative group transition-all duration-300 ${isCurrentUser
      ...
      {/* Input Area */}
      <div className={`${isMobile ? 'px-4 pb-6 pt-3' : 'px-6 pb-8 pt-4'}`}>
